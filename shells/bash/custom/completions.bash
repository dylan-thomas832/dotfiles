...
### NOTE: Git completions should be included for WSL or Standard Linux.
###         Script is explicitly included for cygwin/msys terminals on Windows
...
# pip completions from `pip completion -b`
...
# cargo completions from `rustup completions bash cargo`
source $(rustc --print sysroot)/etc/bash_completion.d/cargo

# rustup completions from `rustup completions bash`
_rustup() {
    ...
    case "${cmd}" in
        rustup)
            opts=" -v -q -h -V  --verbose --quiet --help --version  <+toolchain>  dump-testament show install uninstall update check default toolchain target component override run which doc man self set completions help  docs"
        ...
        rustup__self__upgrade__data)
            ...
            ;;
        rustup__set)
            opts=" -h -V  --help --version   default-host profile help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
